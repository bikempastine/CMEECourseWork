...
model_fit <- data.frame(matrix(ncol = 8, nrow = 0)) #dataframe to store your results from the linear model, each row is another curve
colnames(model_fit) <- c('ID', 'AIC', 'intercept', 'polly1', 'polly2', 'sample_size', 'r_squared', 'AICc')

for (i in 1:length(d)){
    m<- (lm(data=d[[i]], PopBio ~ poly(Time, 2, raw =T)))
    ...
    model_fit[i,6] <- nrow(d[[i]]) #sample size
    model_fit[i,7] <- calc_R2(m, d[[i]]$PopBio) #r_squared
    model_fit[i,8] <- calc_AICc(m, d[[i]]$PopBio) # AICc
...
#save the results
saveRDS(model_plot, file="../data/model_plot_data_quadratic.RData")
saveRDS(model_fit, file="../data/model_fit_quadratic.RData")
